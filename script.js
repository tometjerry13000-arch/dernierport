...
generatePart1() {
    ...
    const alienTransformations = this.generateAlienTransformations(); // ← Gardé si mode Alien en partie 1
    // const avatarTransformations = this.generateAvatarTransformations(); // ← SUPPRIMÉ !
    const specialFeatures = this.generateSpecialFeatures(); // ← Options spéciales (yeux/taches) peuvent rester
...
14. FINALE SPECTACULAIRE : À la 4ème seconde, elle exécute son geste final face au public, maintient la pose, puis effet de fin choisi${avatarLine}${specialLine}`;
   ...
   generateConsignes() {
    ...
    if (avatarMode) {
        if (keepFace) {
            avatarLine = '\n15. ⚠️ MODE AVATAR ACTIVÉ EN PARTIE 2 SEULEMENT : Le visage reste STRICTEMENT IDENTIQUE à l\'image de référence (mêmes traits, mêmes imperfections), seul le corps devient Na\'vi APRÈS la transition.';
        } else {
            avatarLine = '\n15. ⚠️ MODE AVATAR ACTIVÉ EN PARTIE 2 SEULEMENT : Transformation complète en Na\'vi (peau bleue, traits alien) APRÈS la transition.';
        }
    }
    
    let specialLine = '';
    if (eyesDiff && skinDiff) {
        specialLine = '\n16. OPTIONS SPÉCIALES 8K : Yeux de couleurs différentes (hétérochromie) + Taches de naissance/vitiligo - Rendu macro 8K (UNIQUEMENT EN PARTIE 2 APRÈS TRANSITION)';
    } else if (eyesDiff) {
        specialLine = '\n16. OPTIONS SPÉCIALES 8K : Yeux de couleurs différentes (hétérochromie) - Rendu macro 8K avec vaisseaux sanguins visibles (UNIQUEMENT EN PARTIE 2 APRÈS TRANSITION)';
    } else if (skinDiff) {
        specialLine = '\n16. OPTIONS SPÉCIALES 8K : Taches de naissance/vitiligo - Rendu hyper-réaliste 8K avec texture de peau préservée (UNIQUEMENT EN PARTIE 2 APRÈS TRANSITION)';
    }
